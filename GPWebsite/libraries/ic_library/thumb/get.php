...
class iCThumbGet
{
	 ...
	 * @since   1.0.0
	 */
	static public function thumbnail($image, $thumbsPath, $subFolder, $width, $height, $quality, $crop = null, $prefix = null, $type = null, $name = null)
	{
		$app = JFactory::getApplication();

		$linkToImage = filter_var($image, FILTER_VALIDATE_URL) ? $image : JPATH_ROOT . '/' . $image;
		...
		if ( ($w * $h * 4) > ($memory_limit - memory_get_usage()) )
		{
			if ( $app->isAdmin() )
			{
...
//		$imgextension = strtolower($fichier[1]); // fixed 3.1.10
			...
			if ($MimeTypeOK)
			{
				...
				if (($imgextension == "jpg") OR ($imgextension == "jpeg") OR ($imgextension == "png") OR ($imgextension == "gif"))
				{
					...
					if (!file_exists(JPATH_ROOT . '/' . $Thumb_Link))
					{
						...
						{
							iCThumbCreate::createThumb($linkToImage, JPATH_ROOT . '/' . $Thumb_Link, $width, $height, $quality, $crop, $prefix);
						}
						else
						{
							$Invalid_Link = true;
			...
			||  $type == 'imgTagLinkModal')
		{
			...
			if ((!file_exists(JPATH_ROOT . '/' . $Thumb_Link)) AND ($image) AND (!$fopen))
			{
		...
		else
		{
			...
			if ( $Invalid_Img_Format
				...
				|| $MimeTypeERROR
				|| $fopen_bmp_error_msg
				|| ((!file_exists(JPATH_ROOT . '/' . $Thumb_Link)) AND ($image)) )
			{
	 ...
	 * @since   1.0.0
	 */
	static public function thumbnailImgTag($image, $thumbsPath, $subFolder, $width, $height, $quality, $crop = null, $prefix = null)
	{
	 ...
	 * @since   1.0.0
	 */
	static public function thumbnailImgTagLinkModal($image, $thumbsPath, $subFolder, $width, $height, $quality, $crop = null, $prefix = null)
	{

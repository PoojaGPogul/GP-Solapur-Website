...
class icagendaCustomfields
{
	...
	static public function getList($id, $parent_form = null, $state = null)
	{
		...
		$query->select('cf.slug AS cf_slug, cfd.value AS cf_value, cfd.parent_id AS cf_parent_id, cf.title AS cf_title')
...
//				if ($custom_fields) $icf->value = $custom_fields[$icf->slug];
				if ( $app->isSite() )
				{
					$icf->value = $custom_fields ? $custom_fields[$icf->slug] : '';
				}
...
//			$cf_fields.= '<select'.$list_required.' id="' . $slug . '" name="' . $ic_data . '['.$slug.']">';
			$cf_fields.= '<select id="' . $slug . '_slug" name="' . $ic_data . '['.$slug.']">';
